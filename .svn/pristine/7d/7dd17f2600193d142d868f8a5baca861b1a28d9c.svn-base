<template>
	<div class="danChe" :style='{height: deviceHeight + "px"}'>
		<van-nav-bar title="为他担保" left-arrow fixed @click-left="leftBack" />
		<div class="topTitle">
			<img src="../../images/cheTitle.png" class="leftImg" />
			<img src="../../images/cheIcon.png" class="rightImg" />
		</div>
		<van-cell-group>
			<van-field v-model="type" label="车辆品牌" :left-icon="require('../../images/car.png')"
				placeholder="请输入车辆品牌" />
			<van-field v-model="nowMoney" clearable label="车辆现价" :left-icon="require('../../images/qian8.png')"
				placeholder="请输入车辆现价" />
			<van-field v-model="buyMoney" label="购买价格" :left-icon="require('../../images/qian9.png')"
				placeholder="请输入购买价格" />
			<van-field v-model="year" label="车辆年限" :left-icon="require('../../images/date5.png')"
				placeholder="请输入车辆年限" />
		</van-cell-group>

		<van-cell-group class="uploadImg">
			<van-field readonly label="车辆照片" :left-icon="require('../../images/pic.png')" />
	...
	export default {
		...
		methods: {
			...
			getDetail(){
				...
				this.$api.oneLoanGuaranteeAssetsDetail({assets_code: this.assetsCode}).then(res => {
					...
					if (res.errno == 0) {
						...
						this.buyMoney = res.data.assets_last_price
			...
			updete(){
				...
				if(this.buyMoney.trim().length == 0){
					this.$toast("请输入购买价格")
					return
				}
				...
				data.assets_last_price = this.buyMoney // 购买价格
