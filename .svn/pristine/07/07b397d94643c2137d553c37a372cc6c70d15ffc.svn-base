<template>
	<div class="depositBillList" >
        <div class="head-con">
            <van-nav-bar title="押金明细" :border="false" left-arrow @click-left="leftBack"/>
            <div class="banner" style="height:200px" ></div>
            <div class="head-absolute-center">
                <div class="desc">剩余押金</div>
                ...
                <van-list v-model="loading" :finished="finished" :finished-text="list && list.length > 0 ? '没有更多了' : ''" @load="getDetail" >
                    ...
                    <div class="list-item-con" :class="{'van-hairline--bottom': (index+1 != list.length) }" v-for="(item,index) in list" :key="index" @click="toDetail(item)">
                        ...
                        <div class="order-info">
                            <div class="tit">{{(item.type == 1 ? '充值' : (item.type == 4 ? '支出' : (item.type == 5 ? '退款' : '充值'))) + (item.str ? ('-'+item.str) : '')}}</div>
                            <div class="no" v-if="item.desc">{{item.desc}}</div>
                            <div class="date">{{item.add_time}}</div>
                        </div>
                        <div class="price">
                            <span :class="{'add': item.status == 1 && (item.type == 1 || item.type == 2)}">{{(item.status == 2 || item.status == 4 || item.type == 5) ? '' : (item.type == 1 || item.type == 2 ? '+' : '-')}}{{item.money}}</span>
                            <span class="status" :class="{'nopay': item.status == 4, 'shenhe': item.status == 3}" v-if="item.status != 1">
                                {{item.type == 1 ? '' : (item.type == 4 ? '支出' : (item.type == 5 ? '退款' : ''))}}
                                <template v-if="item.status == 2">
                                   <template v-if="item.cls == 2">
                                       <span style="color:#000">已退款</span>
                                   </template>
			...
			<template v-if="(!list || list.length == 0 || isErr) && !loading">
				<div class="no-data">
					<div class="no-data-con">
						...
						<p class="no-title">{{ isErr ? '网络竟然崩溃了' : '暂无押金明细'}}</p>
						...
						if (data && data.length > 0) {
                            data.forEach(item =>{
                                if (item.type == 1 || item.type == 5) {
                                    if (item.cls == 1) {
                                        item.desc = item.type == 1 ? '从微信扣款' : '退款至微信'
                                    } else if (item.cls == 2) {
                                        item.desc = item.type == 1 ? '从银联扣款' : '退款至银行卡'
                                    } else if (item.cls == 3) {
                                        item.desc = item.type == 1 ? '从社内余额扣款' : '退款至社内余额'
                                    }
                                } 
                                item.str = item.title
                            })
