<template>
	<div class="addTask">
        ...
        <!-- <div class="header-con">
            <img class="bg" src="../../images/bountyTask/send-top-bg.png" alt="">
        ...
        <van-popup class="type-popup-con" v-model="isShowTypePopup" round position="bottom"  style="max-height:80%" >
...
<script>
	export default {
		...
		mounted() {
            ...
            //     this.stepList = []
            // }
            if (localStorage.getItem('taskForm') || localStorage.getItem('stepList')) {
                let taskForm = JSON.parse(localStorage.getItem('taskForm'))
                let tmpStepList = localStorage.getItem('stepList') ? JSON.parse(localStorage.getItem('stepList')) : []
                if (taskForm.typeName || taskForm.title || taskForm.name || taskForm.note || taskForm.address || taskForm.price || taskForm.num || (tmpStepList && tmpStepList.length > 0)) {
		...
		methods: {
            leftBack() {
                if (this.typeName || this.title || this.name || this.note || this.address || this.price || this.num) {
                    this.setStorage()
                    setTimeout(() =>{
            ...
            bindInput (type) {
                if (type == 'price') {
                    this.price = this.handlePrice(this.price)
                } else if (type == 'digit') {
