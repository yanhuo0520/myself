<template>
	<div class="index">
		...
		<div class="card">
			...
			<p class="daiMoney" @click.stop="$router.push('/accountIndex')">{{accountNum.wallet ? accountNum.wallet : 0}}</p>
			<div class="module">
				<div class="item" @click.stop="$router.push('/creditScore')">
					<p class="num">{{accountNum.credit_score ? accountNum.credit_score : 0}}</p>
					<p class="title">信用评分</p>
				</div>
				<div class="item" @click="$router.push('/cooperativeAcc')">
		...
		<van-grid class="grid" :column-num="4" :border="false">
			<van-grid-item v-for="(item, index) in grid" :key="index" @click="goGridItem(item.path, item.id)">
		...
		<div class="fuli">
			...
			<!-- 赏金任务 -->
			<div class="right" @click="$router.push('/chooseTaskArea')">
	...
	export default {
		...
		mounted() {
			...
			if(typeof android != 'undefined'){
				...
				this.versionId = android.load("versionId")
				this.locationAddress = android.load("locAddress")
		...
		methods: {
			...
			// 用户首页个人信息
			userIndex() {
				this.$api.user_index({upgrade_id: this.versionId}).then(res => {
					if (res.errno == 0) {
						this.accountNum = res.data;

						if(res.version_info && (typeof android != 'undefined')){
							android.save("versionId", res.version_info.id);
							this.$dialog.confirm({
								title: '版本更新提醒',
								message: res.version_info.upgrade_point
							}).then(()=>{
								android.del('token')
        ...
        locGD() {
           ...
           android.requestLocation();
			...
			locationGDAll(location){
				...
				android.save("locAddress", this.locationAddress);
			},
			cancelDialog(){
				this.$toast.clear()
