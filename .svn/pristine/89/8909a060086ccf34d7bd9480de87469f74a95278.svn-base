	...
	<div class="applyJie">
		...
		<div class="groupCont">
			...
			<van-cell-group>
				...
				<van-field v-model="jieNum" type="number" clearable label="借款金额" :left-icon="require('../../images/jieNum.png')"
		...
		<!-- 选择借款时长 -->
		<van-action-sheet v-model="selectShow1" title="选择借款时长" class="selSheet" :closeable="false" cancel-text="确定" @cancel="sureSel1()">
		...
		<!-- 借款用途 -->
		<van-action-sheet v-model="selectShow2" title="选择借款用途" class="selSheet" :closeable="false" cancel-text="确定" @cancel="sureSel2()">
		...
		<!-- 还款方式 -->
		<van-action-sheet v-model="selectShow3" title="选择还款方式" class="selSheet" :closeable="false" cancel-text="确定" @cancel="sureSel3()">
		...
		<!-- 担保人 -->
		<van-action-sheet v-model="selectShow4" title="选择担保人" class="selSheet selSheet4" :closeable="false" cancel-text="确定" @cancel="sureSel4()">
...
<script>
	export default {
		...
		methods: {
			...
			// 我要借款--获取借款用途
			getLoanUse() {
				this.$api.get_loan_use().then(res => {
					if (res.errno == 0) {
			...
			// 我要借款--获取还款方式
			getRepayment() {
				this.$api.get_repayment().then(res => {
					if (res.errno == 0) {
			...
			getInterestByRate() {
				...
				this.$api.get_interest_by_rate(data).then(res => {
					if (res.errno == 0) {
			...
			selectGuarantee() {
				...
				this.$api.select_guarantee(data).then(res => {
					if (res.errno == 0) {
			...
			sendApply() {
				...
				this.$api.submit_loan_apply(data).then(res => {
					if (res.errno == 0) {
						this.$toast("申请已提交，请耐心等待担保审核")
						setTimeout(()=>{
							this.$router.go(-1);
						}, 1000)
					} else {
