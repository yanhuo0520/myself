	...
	<div class="joinCooper" :style='{height: deviceHeight + "px"}'>
		...
		<div class="itemCont itemCont3" v-if="itemId == 2">
			...
			<van-cell-group>
				...
				<van-field v-model="idNumber" readonly clearable label="身份证号" @touchstart.native.stop="keywordId = true" :left-icon="require('../../images/person2.png')"
					placeholder="请输入身份证号" />
				<van-number-keyboard
					:show="keywordId"
					v-model="idNumber"
		...
		<!-- 选择民族 -->
		<van-action-sheet v-model="selecNationShow" title="选择民族" class="selSheet" :closeable="false" cancel-text="确定" @cancel="sureSel(1)">
		...
		<!-- 选择学历 -->
		<van-action-sheet v-model="selectEduShow" title="选择学历" class="selSheet" :closeable="false" cancel-text="确定" @cancel="sureSel(2)">
		...
		<!-- 选择政治面貌 -->
		<van-action-sheet v-model="selectZZShow" title="选择政治面貌" class="selSheet" :closeable="false" cancel-text="确定" @cancel="sureSel(3)">
		...
		<!-- 选择从事行业 -->
		<van-action-sheet v-model="selectHYShow" title="选择从事行业" class="selSheet" :closeable="false" cancel-text="确定" @cancel="sureSel(4)">
...
<script>
	export default {
		...
		methods: {
			...
			userInfoDetail(){
				this.$api.user_info_detail().then(res => {
						this.$toast.clear()
						if (res.errno == 0) {
			...
			goNext(id) {
				if (id == 1) {
					if (this.seltype == 0 && this.cooperation_id.length == 0) {
						this.$toast("请选择想要加入的合作社")
						return
					} else if (this.seltype == 1 && this.cooperation_id.length == 0) {
					...
					this.$api.newUserAuth(data).then(res => {
						...
						if (res.errno == 0) {
							if(this.userName != this.name){
								if(typeof android != "undefined"){
									android.save("userName", this.name);
								}else{
			...
			// 身份证头像面
			afterRead1(file){ // 判断图片大小，确定是否压缩图片
