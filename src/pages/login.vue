<template>
	<div class="login">
		...
		<div class="loginGroup">
			<p class="title">手机号</p>
			<!-- fsdfsdfsdfsdf sdf sadf sd  -->
			<van-field v-model="phone" type="number" :class="ifInPhone? 'ifInPhone' : ''" clearable
			...
			<van-field v-model="yzCodePw" @input="filterSpaces(1)" clearable :placeholder="ifPlaceholder">
				<!-- 发送验证码/  忘记密码 -->
		...
		<van-popup v-model="bandPhone" class="tipCont">
			...
			<div class="inCont">
				<van-field v-model="phoneBand" type="number" clearable placeholder="请输入手机号" />
				<van-field v-model="yzmBand" type="number" clearable placeholder="请输入手机验证码" class="yzmCell">
		...
		<van-popup v-model="setPassword" class="tipCont">
			...
			<div class="inCont">
				<van-field v-model="setPassword1" @input="filterSpaces(2)" clearable placeholder="请输入密码" />
				<van-field v-model="setPassword2" @input="filterSpaces(3)" clearable placeholder="请再次输入密码" />
...
<script>
	export default {
		...
		methods: {
			...
			filterSpaces(id){
				if(id == 1){
					this.yzCodePw = this.yzCodePw.replace(/ /g,'')
				}else if(id == 2){
					this.setPassword1 = this.setPassword1.replace(/ /g,'')
				}else if(id == 3){
					this.setPassword2 = this.setPassword2.replace(/ /g,'')
				}
			...
			WxLogin() {
				this.phoneBand = ''
				this.yzmBand = ''
				if (typeof android != "undefined") { // 调用Android方法唤起微信登录
					android.loginWX()
					return
				} else {
					this.$toast("请用手机端登录")
					...
					this.$api.wechatLogin({
						...
						if (res.errno == 0) {
							if(typeof android != "undefined"){
								android.save("token", res.token);
								android.save("userName", res.name);
								android.save("phone", res.phone);
							}else{
			...
			login() {
				...
				if (this.$validatePassword(this.yzCodePw) != true && this.showRember) {
					this.$toast(this.$validatePassword(this.yzCodePw))
					return
				}
				if (!this.ifNewUser && !this.isnoPawUser) { // 密码登录 
					this.$api.login({
						...
						if (res.errno == 0) {
							...
							if(typeof android != "undefined"){
								android.save("token", res.token); // 避免链接跳转导致的缓存丢失的问题
								android.save("userName", res.name);
								android.save("phone", res.phone);
							}else{
								localStorage.setItem('token', res.token)
							...
							if(this.ifRemember){
								if(typeof android != "undefined"){
									android.save("loginPhone", this.phone);
									android.save("loginPas", this.$Base64.encode(this.yzCodePw));
								}else{
								...
								if(typeof android != "undefined"){
									android.del('loginPhone') 
									android.del('loginPas') 
								}else{
		...
		created(){
			...
			let cookPass = ''
			if(typeof android != "undefined"){
				cookPhone = android.load("loginPhone")
				cookPass = android.load("loginPas")
			}else{
				cookPhone = this.$Cookies.get('loginPhone')
				cookPass = this.$Cookies.get('loginPas')
			}
			if(cookPhone){
