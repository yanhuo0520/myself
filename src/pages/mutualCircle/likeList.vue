<template>
	<div class="likeList" >
		<van-nav-bar :border="false"  title="我的点赞" left-arrow @click-left="leftBack"/>
...
<script>
	export default {
        ...
        activated() {
            ...
            if (lastPath == '/mutualCircle') {
                ...
                if (meta && meta.scrollPosition && Number(meta.scrollPosition.y) > 100) {
                    document.querySelector('.likeList').scrollTop = meta.scrollPosition.y
                } else  {
                    this.page = 0
                    this.loading = true
                    this.finished = false
        ...
        beforeRouteLeave (to, from, next) {
            from.meta.scrollPosition.y = document.querySelector('.likeList').scrollTop
			next()
	    },
		methods: {
			// 返回
            ...
            getData () {
				...
				this.$api.mygiveLists(params).then(res => {
                    this.loading = false
