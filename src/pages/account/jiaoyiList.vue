<template>
	<div class="jiaoyiList" @scroll="scrollEvent($event)" :style='{height: deviceHeight + "px"}'>
		...
		<div class="content" v-if="list.length > 0">
			<div class="item" v-for="(item, index) in list" :key="index" @click="goDetail(item.status, item.w_id)">
				<div class="left">
					<!-- "log_type": 3,//记录类型：0全部 1存款 2取款 3活期利息 4赏金转入 -->
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				this.$api.user_wallet_log(data).then(res => {
					this.$toast.clear()
					if (res.errno == 0) {
