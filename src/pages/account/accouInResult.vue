		...
		<div class="huanCont">
			...
			<template v-if="ifSuccess == 1">
				...
				<div class="cont">
					...
					<div class="item" v-else-if="payData.pay_type == 2 || payData.pay_type == 5">
						<img src="../../images/wei.png" />
						<p class="name">微信</p>
					</div>
					<div class="item" v-else-if="payData.pay_type == 3 || payData.pay_type == 6">
...
<script>
	export default {
		...
		methods: {
			leftBack() {
				// this.$router.go(-1);
				if(this.$route.query.token){ // 支付宝支付返回页面地址为线上地址，需要跳转Android本地打包地址
					if(typeof android != 'undefined'){
						android.localUrl('/index.html#/accountIndex')
					}
				}else{
