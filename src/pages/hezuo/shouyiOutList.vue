<template>
	<div class="shouyiOutList" @scroll="scrollEvent($event)" :style='{height: deviceHeight + "px"}'>
		...
		<div class="content" v-if="list.length > 0">
			<div class="item" v-for="(item, index) in list" :key="index">
				<div class="left">
					...
					<div>
						<p class="title">转出到{{item.new_turn_bank_name}}({{item.turn_bank_num}})
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				this.$api.passbook_bonus_out_log(data).then(res => {
					this.$toast.clear()
					if (res.errno == 0) {
						this.totalNum = res.data.count;
						this.totalPage = res.data.total_page
						this.monthMoney = res.data.month_out;
						this.totalMoney = res.data.all_out;
						...
						logList.forEach(ele => {
							// 过滤掉中国两个
							ele.new_turn_bank_name = ele.turn_bank_name.replace("中国", "");
						})
