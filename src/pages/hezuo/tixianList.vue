<template>
	<div class="tixianList" @scroll="scrollEvent($event)" :style='{height: deviceHeight + "px"}'>
		<van-nav-bar title="提现记录" left-arrow fixed @click-left="leftBack" />
		<div class="topTitle">
			<p class="time" @click="show=true">{{showTime}}</p>
			<p>本月提现￥{{monthMoney}} 累积提现￥{{totalMoney}}</p>
		</div>
		<div class="content" v-if="list.length > 0">
			<div class="item" v-for="(item, index) in list" :key="index">
				<div class="left">
					<img src="../../images/tixianICon.png" />
					<div>
						<p class="title">提现<span v-if="withdrawal_type == 1">到微信</span><span v-else>到平台</span>
							<van-tag round class="faile" v-if="item.withdrawal_status == 2">失败</van-tag></p>
						<p class="time">{{item.withdrawal_date}}</p>
					</div>
				</div>
				<p class="price">{{item.withdrawal_money}}</p>
			</div>
			<!-- 加载中 -->
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				this.$api.userTeamWithdrawaLog(data).then(res => {
					this.$toast.clear()
					if (res.errno == 0) {
						this.totalNum = res.data.count;
						this.totalPage = res.data.total_page
						this.monthMoney = res.data.month_withdrawal_money;
						this.totalMoney = res.data.total_withdrawal_money;
